...
- **No `agents/__init__.py`**: The agents package has no `__init__.py`; imports are by submodule path (e.g., `loom.agents.coding_agent`). New agent-layer types should follow this pattern
...
| File | Purpose |
|------|---------|
| `src/loom/connectors/interaction.py` | `UserInteraction` protocol, `UserQuestion`, `UserResponse`, `CLIUserInteraction`, `CallbackUserInteraction`, `SuspendUserInteraction` |
| `src/loom/connectors/oauth_providers.py` | `OAuthProviderConfig`, provider registry, 9 built-in providers, `discover_oidc()` |
| `src/loom/runtime/environment.py` | `RunEnvironment`, `current_run_environment()` ContextVar |
...
| File | Changes |
|------|---------|
| `src/loom/connectors/__init__.py` | Re-export `UserInteraction`, `UserQuestion`, `UserResponse`, `CLIUserInteraction`, `CallbackUserInteraction`, `OAuthProviderConfig`, `register_oauth_provider` |
| `src/loom/connectors/credentials.py` | Add `LayeredCredentialStore` class |
| `src/loom/runtime/engine.py` | Add `env` to `Runtime.__init__`. Add `credentials`, `env` to `run()` and `submit()`. Build layered store in `_drive_inner` |
| `src/loom/runtime/context.py` | Add `self.env: RunEnvironment`. Bind env ContextVar in `_attempt_loop`. Import `RunEnvironment` |
| `src/loom/toolsets/connections.py` | Add `tokens` param to `ConnectionBroker.__init__` and priority lookup |
| `src/loom/agents/coding_agent.py` | Add `user_interaction` param to `WorkflowCodingAgent.__init__`, pass to `build_coding_tools()` |
| `src/loom/agents/coding_tools.py` | Add `ask_user` tool. Extend `build_coding_tools()` signature with `user_interaction` param |
| `src/loom/agents/agent.py` | Add optional `user_interaction` field to `Agent` |
| `src/loom/agents/runner.py` | Thread `user_interaction` to inject `ask_user` tool |
| `src/loom/cli/auth_commands.py` | Integrate `get_oauth_provider()` in `_resolve_target`. Add `cmd_providers` |
| `src/loom/cli/commands.py` | Add `--env` / `--env-file` to run parser. Register `providers` subcommand |
| `src/loom/facade.py` | Extend `LocalFacade.start()` to accept `credentials` and `env` |
| `src/loom/toolsets/google/auth.py` | Use `current_run_environment()` in `GoogleCredentials.from_env()` as fallback |
| `src/loom/__init__.py` | Export `RunEnvironment`, `UserInteraction` (if public API desired) |
