...
def blob_backend_from_url(url: str, **kwargs: Any) -> BlobBackend:
    ...
    if scheme == "s3":
        try:
            ...
            raise ConfigurationError(
                "S3BlobBackend needs aioboto3: pip install 'workflow-builder[s3]'"
            ) from exc
    ...
    if scheme in ("az", "azure"):
        try:
            ...
            raise ConfigurationError(
                ...
                "pip install 'workflow-builder[azure]'"
            ) from exc
        from workflow_builder.storage.blob_azure import AzureBlobBackend
    ...
    if scheme in ("gs", "gcs"):
        try:
            ...
            raise ConfigurationError(
                ...
                "pip install 'workflow-builder[gcs]'"
            ) from exc
        from workflow_builder.storage.blob_gcs import GCSBlobBackend
