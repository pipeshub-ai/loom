...
:class:`~workflow_builder.connectors.credentials.CredentialStore`.
...
``metadata`` for :class:`~workflow_builder.connectors.oauth_client.MetadataRefresher`
...
from workflow_builder.cli.commands import printer_for, run_async
from workflow_builder.cli.output import Exit, Printer
from workflow_builder.connectors.oauth_client import OAuthTokenError, generate_pkce_pair
from workflow_builder.core.exceptions import AuthExpired, ConfigurationError, CredentialNotFound

if TYPE_CHECKING:
    from workflow_builder.connectors.credentials import CredentialStore, StoredCredential
    from workflow_builder.connectors.oauth_client import DeviceAuthorization, OAuthClient
...
def _store() -> CredentialStore:
    ...
    """
    from workflow_builder.connectors.credentials import KeyringCredentialStore
...
def cmd_whoami(args: argparse.Namespace) -> int:
    ...
    async def body() -> int:
        from workflow_builder.connectors.credentials import Peekable
        from workflow_builder.runtime.clock import SystemClock
