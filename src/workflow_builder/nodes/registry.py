...
from workflow_builder.nodes.base import Node, near_matches
from workflow_builder.nodes.catalog import NodeCatalog
from workflow_builder.nodes.errors import NodeContractError, NodeNotFound
from workflow_builder.nodes.spec import NodeSpec
...
class NodeRegistry(NodeCatalog):
    ...
    def _requirement_error(spec: NodeSpec, missing: list[str]) -> Exception:
        ...
        if "human_channel" in missing:
            from workflow_builder.nodes.errors import HumanChannelMissing
...
#: of a registry rather than at ``import workflow_builder``.
#:
...
_BUILTIN_NODE_MODULES = (
    "workflow_builder.nodes.guard.nodes",
    "workflow_builder.nodes.human.nodes",
    "workflow_builder.nodes.stdlib",
)
