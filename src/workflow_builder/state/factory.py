...
def from_url(url: str) -> Any:
    ...
    if scheme in ("postgres", "postgresql"):
        try:
            ...
            raise ConfigurationError(
                "PostgresStore needs asyncpg: pip install 'workflow-builder[postgres]'"
            ) from exc
    ...
    if scheme in ("mongodb", "mongodb+srv"):
        try:
            ...
            raise ConfigurationError(
                "MongoStore needs motor: pip install 'workflow-builder[mongo]'"
            ) from exc
        database = parsed.path.lstrip("/") or "workflow_builder"
