...
made elsewhere: :func:`~workflow_builder.identity.scopes.scopes_to_grant`
narrows a workflow's grant by it, and :class:`~workflow_builder.identity.facade.AuthorizedFacade`
...
class _VerifiedToken(Protocol):
    ...
    under :mod:`workflow_builder.identity` has a reason to depend on the
...
class Principal:
    ...
    consulted by anything in :mod:`workflow_builder.identity` itself, so a
    ...
    def has(self, scope: str) -> bool:
        ...
        :func:`~workflow_builder.security.grants._covers` uses for
        ``GrantSet`` toolset entries, so one scope vocabulary and one
        matching rule serves both the inbound (this) and outbound
        (``GrantSet``) sides.

        ``"admin"`` (:data:`~workflow_builder.identity.scopes.Scope.ADMIN`)
        satisfies every check, the same wildcard
        :func:`~workflow_builder.identity.scopes.scopes_to_grant` gives it
...
:class:`~workflow_builder.identity.facade.AuthorizedFacade` with this
