...
- **LOOM-surface scopes** (:class:`Scope` below — ``runs:write``,
  ...
  via :meth:`~workflow_builder.identity.principal.Principal.requires` in
  :class:`~workflow_builder.identity.facade.AuthorizedFacade`.
- **Toolset-shaped scopes** (``jira.issues:read``, ``slack.chat:write``, the
  same vocabulary a workflow's own :class:`~workflow_builder.security.grants.GrantSet`
