...
:mod:`loom.mcp_server.auth` gets for free from
...
``mcp_server/auth.py`` via :mod:`loom.identity.verifier`, which
for the same reason returns LOOM's own :class:`~loom.identity.verifier.VerifiedToken`
...
if TYPE_CHECKING:
    ...
    from loom.identity.verifier import TokenVerifier
