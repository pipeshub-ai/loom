...
``OAuthClient`` also implements :class:`~loom.connectors.credentials.Refresher`,
...
- **Single-flight across processes.** Refresh tokens are commonly rotated on
  ...
  under load. A :class:`~loom.stores.base.LockProvider` lease
...
class _StoreLike(Protocol):
    ...
    Narrower than :class:`~loom.connectors.credentials.CredentialStore`
...
class OAuthClient:
    ...
    *store* and *lock* are both optional and both needed for cross-process
    ...
    async def refresh(self, name: str, stored: StoredCredential) -> StoredCredential:
        ...
        :class:`~loom.connectors.credentials.Refresher`."""
...
class MetadataRefresher:
    """One :class:`~loom.connectors.credentials.Refresher` for
    ...
    initial :meth:`~loom.connectors.credentials.CredentialStore.put`.
