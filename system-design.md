...
**loomsdk** (codename LOOM) is a pip-installable, library-first durable execution SDK whose
...
```bash
pip install loomsdk       # only requires: pydantic, pydantic-settings
...
```bash
pip install loomsdk[postgres]    # adds asyncpg
pip install loomsdk[mongodb]     # adds motor
pip install loomsdk[temporal]    # adds temporalio
```
...
The builder is itself a session agent: `Agent("loomsdk", persistence="session")` whose
...
**Goal:** `pip install loomsdk`, a working durable execution engine with the correct
